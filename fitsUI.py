...
class Plotter:
    ...
    def update_zij_plot(self, coordinates, projection, specimen, lines=[], planes=[], plot_output_area=None):
        ...
        with plot_output_area:
            clear_output(wait=True)
            fig = plo.zij_plt(coordinates, projection, specimen.get_raw_specimen(), specimen.get_filtered_specimen(), lines, planes)
            fig.tight_layout()
    ...
    def update_linmod_plot(self, coordinates, projection, specimen, lines, fitted_points, coefficients, coefficients_norm, plot_output_area=None):
        ...
        with plot_output_area:
            clear_output(wait=True)
            fig = plo.linzij_plt(coordinates, projection, specimen.get_raw_specimen(), specimen.get_filtered_specimen(), lines, fitted_points, coefficients, coefficients_norm)
            fig.tight_layout()
            fig.canvas.header_visible = False
            fig.canvas.footer_visible = False
            plt.show()
